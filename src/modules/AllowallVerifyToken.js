...
function verifyToken(req, res, next) {
    ...
    jwt.verify(token, config.secret, function(err, decoded) {
        ...
        var iptv_flag = false;
        var app_flag = true;
        Subscription.findOne({where:{subscription_id:decoded.subscription_id,expires_on:{[Op.gt]:current_date}},include:[SubscriptionBundle]}).then(function(customer){
            if(customer){
		req.mobile=customer.mobile;
		    ...
		    if(accesslogin || req.device_type=='STB'){
                    if(req.device_type=='STB'){
                        app_flag = (customer.allowed_device = 'mobile+tv+stb') ? true : false
                        provider_data(app_flag);
                    }else{
                        if(customer.allowed_device = 'mobile+tv'){
                            provider_data(app_flag);
                        }else{
                            app_flag = (customer.app) ? true : false
                            provider_data(app_flag);
                        }
                    }
                                    ...
                                    ProviderScType.findAll({where:{subscription_id:customer.subscription_id}}).then(function(available_providers){
                                            ...
                                            for (var i = all_providers.length - 1; i >= 0; i--) {
                                                ...
                                                if(!unallowed_providers.some(function(item){ return item.provider_id == all_providers_provider_id }) && !avail_provider_obj[all_providers_provider_id]){
                                                    var final_obj={
                                                        ...
                                                        "expire_on":new Date(),
                                                        "method":all_provider_index.method.toLowerCase().includes('link')?'Link':'Direct',
                                                        ...
                                                        "allowed_service_types":operatorsetting && operatorsetting.allow_operator_content && all_provider_index.brand=='operator'?['MV','LIVE']:[],
                                                        "message":msg
                                                    }
                                        ...
                                        if(!app_flag){
                                            for(var i in avail_provider_obj){
                                                ...
                                                unallowed_providers.push(object);
                                            }
