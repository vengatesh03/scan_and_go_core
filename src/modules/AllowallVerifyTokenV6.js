var jwt = require('jsonwebtoken'),
    ...
    ProviderScTypePackage = query_info.ProviderScTypePackage,
    Org = __db_model.Org,
...
function verifyToken(req, res, next) {
    ...
    jwt.verify(token, config.secret, function (err, decoded) {
        ...
        Subscription.findOne({
          where: { subscription_id: decoded.subscription_id }, 
          include: [{
            model:SubscriptionBundle,
            where:{
                [Op.or]: [
                    { addon: 1, addon_status : {[Op.ne]:'progress'} },
                    { base: 1 }
                ]
            }
          }] 
	    ...
	    if(!customer) { return res.status(403).send("No subscriptions found. Please contact your operator") }
            ...
            if (customer && customer.status != 'Pending' && (customer.mobile==decoded.phone_number)) { 
                req.mobile = customer.mobile;
                req.expiry_date = customer.expires_on;
                req.subscription_mode = customer.mode;
                ...
                AccessLogin.findOne({ raw: true, where: { subscription_id: decoded.subscription_id, device_id: decoded.device_id, device_type: req.device_type } }).then(function (accesslogin) {
                    if (accesslogin || req.device_type == 'STB') {
                        ...
                        Bundle.findOne({ raw: true, where: { bundle_id: arr } }).then(function (sub_bundle) {
			                req.base_bundle_name=sub_bundle.bundle_name;
                            if(sub_bundle.bundle_type=='resellerpackage'){
                                req.bundle_cost=sub_bundle.seller_cost;
                            }
                            else{
                                req.bundle_cost=sub_bundle[keyObj[sub_bundle.bundle_mode]];
			                }
                            var stbAppBundleFlag = (sub_bundle.allowed_device == 'stb_app') ? true : false;
                            var appBundleFlag = (sub_bundle.allowed_device == 'only_app') ? true : false;
                            var stbBundleFlag = (sub_bundle.allowed_device == 'only_stb') ? true : false;
                                ...
                                OperatorSetting.findOne({ raw: true, where: { org_id: customer.org_id } }).then(function (operatorsetting) {
                                    Org.findOne({ raw: true, where: { org_id: customer.org_id } }).then(function (org) {
                                        ...
                                        Provider.findAll({ include: [{ model: DirectAccess }, { model: LinkAccess }], attributes: ['provider_id', 'method', 'brand', ['attach_logo', 'logo_attached'], ['logo_url', 'logo']] }).then(function (all_providers) {
                                            ProviderScTypePackage.findAll({ where: { subscription_id: customer.subscription_id } }).then(function (available_providers) {
                                                ...
                                                if (available_providers && available_providers.length > 0) {
                                                    for (var j = available_providers.length - 1; j >= 0; j--) {
                                                        ...
                                                        if (avail_provider_index.sc_type.length > 0) {
                                                            var str = '';
                                                            avail_provider_index.sc_type.map(function (sc_data) {
                                                                if (str == '') {
                                                                    str = sc_data.id;
                                                                } else {
                                                                    str = str + ',' + sc_data.id;
                                                        ...
                                                        var sc_type = str.replace(/ /g, '').split(',');
